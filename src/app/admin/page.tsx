    ...
    try {
                        ...
                        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                            {orders.length === 0 ? (
                                ...
                                orders.map((order: any) => {
                                    ...
                                    const orderDate = order.createdAt ? format(order.createdAt, 'MMM d, yyyy h:mm a') : 'N/A';
                                    return (
                                        <div key={order.id} className="p-4 bg-secondary/30 rounded border border-white/5 flex flex-col gap-2">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-xs text-muted-foreground">{order.user?.email || order.guestEmail || 'Guest'}</p>
                                                </div>
                                            ...
                                            <div className="border-t border-white/5 pt-2 mt-1">
                                                {items.map((item: any, idx: number) => {
                                                    const itemName = item.product?.name || item.ticketType?.name || 'Unknown Item';
                                                    return (
                                                        <div key={idx} className="flex justify-between text-xs">
                                                            <span className="text-white/80">{itemName} x{item.quantity}</span>
                                                            <span>${((item.priceAtTime || 0) / 100).toFixed(2)}</span>
                                                        </div>
                                                    )
                                                })}
        ...
        return (
            <div className="p-8 text-red-500 bg-black min-h-screen">
                ...
                <p className="font-mono text-sm mb-4">This error was caught by the Safety Net.</p>
